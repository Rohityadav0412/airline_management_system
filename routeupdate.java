...
public class routeupdate extends HttpServlet {
       protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        String query,depart_locn,route_id;
        int count;
        try (PrintWriter out = response.getWriter()) {
            Class.forName("oracle.jdbc.driver.OracleDriver");
            con = DriverManager.getConnection("jdbc:oracle:thin:@localhost:1521:xe","system","rohit");
            query ="update route set depart_locn=? where route_id=?";
            pst = con.prepareStatement(query);
            route_id = request.getParameter("rroute_id");
            depart_locn = request.getParameter("rdepart_locn");
            pst.setString(1, depart_locn);
            pst.setString(2, route_id);
